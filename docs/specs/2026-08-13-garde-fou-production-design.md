...
| `demo-app-refonte` | worktree | `feat/pwa-start-url-cockpit` | **19** | `demo-app-prod` |

Les trois sont le **même dépôt git** sur trois branches. Partager la même base
est donc cohérent — ce n'est pas l'anomalie.

L'anomalie est ailleurs : depuis `demo-app-refonte`, un `supabase db reset`
...
ci-dessus : `demo-app-landing` (sur `main`) passe, `demo-app-refonte` et `demo-app`
sont refusés.
...
```
  ...
  supabase-token    client-site   -      client-site
  supabase-token    IronTrack          -      IronTrack
  supabase-token    fourth-app  -      fourth-app
  supabase-token-2  demo-app-prod        PROD   demo-app, demo-app-landing, demo-app-refonte  ⚠
```
...
Cette commande n'est pas un confort. Le 13 août 2026, la question *« client-site
