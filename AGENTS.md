...
| The same question answered in two places | `ctx doctor -Live` tested that exit code from day one; `ctx`, twenty lines away, did not. The correct version existed the whole time — in the command nobody runs daily, while the flawed one shipped in the command everyone types. When two call sites ask one question, they share a function or one of them is already wrong. Sibling of the hand-copied-list row above: same defect, logic instead of data. **Second instance, 19 August 2026:** on a folder owned by no context, `ctx` answered `NO-GO` while `ctx doctor` answered `INFO`. The refusal was the wrong one, and it offered a remedy the user had already applied. Neither verdict had a test. **Third, 24 August 2026 — and the one that says stop patching instances:** same folder, same process, `ctx` returned `GO` while `doctor` returned `PROBLEME` on the git identity. The false verdict was the reassuring one, so it was the one believed. Fixed by closing the FAMILY instead: the decision is one pure function both consume, the doctrine is one table, and a test refuses that any fact carry two verdicts. Closing it found two more instances nobody had reported — `git/remote` and `vercel/session`. |
...
| A backslash swallowed as an escape | `\e`, `\v`, `\a`, `\t`, `\n` and friends become CONTROL CHARACTERS in a shell heredoc, a `printf`, or a non-raw Python string. The result is invisible in a diff and in most editors: `shims\editor.ps1` shipped in `docs/ARCHITECTURE.md` as ESC + `ditor.ps1`, and lived there through a public release. Two more were produced on 19 Aug 2026 while writing the very sentences warning about it. Write Windows paths with forward slashes in prose, use raw strings in tooling, and check afterwards. **That range excludes `\x09`, and `\t` is the most common of them all** — two documents carried TAB + `hird-app` in place of `third-app` from 13 August 2026, and the recommended check could never see them. Since 22 August the suite does it for you: `Aucun caractere avale par un echappement` in `tests/Documentation.Tests.ps1` scans every tracked file for that range **and** for a tab preceded by a non-blank character. A swallowed `\n` or `\` stays invisible to both — those produce perfectly ordinary characters. **And the range itself had a second hole, found 25 Aug 2026.** It stopped at `\x1f` and let the C1 controls through, `\x7f-\x9f`. A Windows path in a non-raw Python string turned `\Backups\2026` into an OCTAL escape: `\202` produced U+0082, mid-path, and the sweep meant to catch it answered *none* — it had the appearance of having looked. A three-digit octal exceeds `\x1f` as soon as its first digit is 1 or more, so the original range could not structurally see that family. Widened, and proved: with the C1 present the test is RED on the new range and GREEN on the old one. |
...
| An option's value can masquerade as a flag | Reading a command line to decide something means reading it the way the tool does. Fourteen `supabase` options take a **separate** value, so in `db reset --profile --local` the word `--local` is the *value of* `--profile` and aims at nothing local. Boolean flags also accept `--flag=true` with an exact, case-sensitive spelling set, and global flags are accepted **before** the subcommand. A parser that reads only the flags it recognises will confidently answer about a command that was never typed. When the reading is uncertain, answer *ambiguous* and let the guard refuse — never answer *absent*. |
| `git config --get-all credential.helper` cannot see a URL-scoped helper | A helper declared as `credential.https://github.com.helper` is invisible to that read, and `--get-urlmatch <url>` is the only one that resolves the scope. Measured 24 August 2026: this machine has no global helper and does have a scoped one, so the obvious command answered *no helper* — which would have announced a blocking prompt exactly where the push leaves in **silence**, under the global account. Wrong in both directions from one read. |
| `git config --show-origin` separates on a TAB | Not on whitespace. Two places here split on `\s`, so `C:/Users/John Doe/.gitconfig` was truncated to `C:/Users/John` — a file that does not exist, printed in the message that tells you which file to correct. The verdict was never wrong, because git renders a repository's own config as a **relative** path (`file:.git/config`) from the root and from a subdirectory alike; only the printed path was. No machine of the author's has a space in these paths, which is exactly why it survived. |
| A test that reads source code reads the comments too | Two ways, both met on 24-25 August 2026 in the same file. A comment **naming** the trap it forbids (`jamais remote.origin.url`) fires an assertion of non-presence; a comment quoting the right command **satisfies** an assertion of presence with no line of code behind it. Blank the comments out — replace them with spaces of equal length so patterns spanning adjacent words survive — and read what remains. And match a symbol as a **whole name**: `Should -Match 'Get-CtxVerdictGitIdentite'` stayed green when the call was renamed to `...IdentiteJAMAIS`, so it caught everything except the likeliest fault, a typo. |
| A green review check that reviewed nothing | On 25 August 2026 the `Sourcery review` check passed in 18 seconds on a pull request nobody had read: the bot's diff budget was spent, and the second reviewer was over its quota too. What it had posted was a *reviewer's guide* — a description of the PR — which nothing distinguishes from a review if you read only the colour of the check. `gh api repos/<org>/<repo>/pulls/<n>/comments --jq 'length'` answers it: `0` means nobody read it. Same shape as measuring that an interceptor is installed rather than that it is reached. The manual read that replaced it that day found a real defect, so the absence was not free. |
| `[IO.File]` and `Set-Location` | .NET methods resolve a relative path against the PROCESS working directory, which `Set-Location` does not change. On 25 Aug 2026 `[IO.File]::ReadAllBytes('DevContext.psd1')` looked in the home folder, threw, and the `WriteAllText` two lines later CREATED a three-byte file there — a BOM and nothing else — while the real manifest sat untouched. Reading failed loudly; writing succeeded quietly in the wrong place, which is the dangerous half. Pass absolute paths to any `[System.IO.*]` call, or use the PowerShell cmdlets, which do follow the location. |
